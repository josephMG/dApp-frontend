...
describe('ConnectWallet component', () => {
  beforeEach(() => {
    ...
    (fetcher as jest.Mock).mockImplementation(jest.fn(() => Promise.resolve({ ok: true })));
  ...
  it('handle connect click fetch failed', async () => {
    (fetcher as jest.Mock).mockImplementation(jest.fn(() => Promise.resolve({ ok: false })));
    await act(async () => {
      render(<ConnectWallet />);
    });
    const button = screen.getByRole('button');
    ...
    waitFor(async () => {
      const verifyRes = await fetcher('abc')
      expect(verifyRes.ok).toEqual(false);
    })
  });
  it('handle connect click catch', async () => {
    const connectData = useConnect();
    (connectData.connectAsync as jest.Mock).mockImplementation(() =>
      Promise.reject(new Error('handle connect click catch'))
    );
